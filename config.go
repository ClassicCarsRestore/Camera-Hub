...
func (element *ConfigST) GetHttpParams(uuid string) (string, string, string, string) {
	if tmp, ok := element.Streams[uuid]; ok {
		return tmp.Domain, tmp.HTTPPort, tmp.Username, tmp.Password
	}
	return "", "", "", ""
}

func (element *ConfigST) GetRtspParams(uuid string) (string, string, string, string) {
	if tmp, ok := element.Streams[uuid]; ok {
		return tmp.Domain, tmp.RTSPPort, tmp.Username, tmp.Password
	}
	return "", "", "", ""
}
...
func loadConfig() *ConfigST {
	...
	for i, v := range tmp.Streams {
		...
		v.URL = "rtsp://" + v.Username + ":" + v.Password + "@" + v.Domain + ":" + v.RTSPPort + "/cam/realmonitor?channel=1&subtype=0"
