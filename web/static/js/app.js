...
fetch('/' + $('#suuid').val() + '/config')
    ...
    .then((text) => {
        ...
        for (line of lines) {
            ...
            let element = document.getElementById(config[0])
            if (element != null)
                element.value = config[1]
        }
    });

function startRecording() {
    fetch('/' + $('#suuid').val() + '/video/start');
    recording = true
...
function changeCamera(camera) {
    if (recording && !confirm("If you change cameras your recording will stop.\nAre you sure you want to change cameras?"))
        return;
    document.location.href = "/player/" + camera;
}

function resetConfigs(config, value) {
    fetch('/' + $('#suuid').val() + '/config/reset')
        .then((response) => response.text())
        .then((text) => {
            let lines = text.split("\r\n");
            for (line of lines) {
...
function changeConfig(config, value) {
    fetch('/' + $('#suuid').val() + '/config/' + config + '/' + value);
}

function cameraControl(direction, action) {
    fetch('/' + $('#suuid').val() + '/move/' + action + '/' + direction + '/' + cameraSpeed.value);
}

function takeSnapshot() {
    fetch('/' + $('#suuid').val() + '/snapshot');
    alert("A snapshot was taken and uploaded in the Pinterest Board.");
}
